...
$('body').on("elegantLoaded", function(){
    ...
    if($('#elegant_video').length != 0)
    {
        $('#elegant_video').mb_YTPlayer();
    }

    //Ripple

    if( $('#elegant_background_ripple').length != 0 )
    {
        $('#elegant_background_ripple').ripples({
            resolution:500,
            dropRadius:25,
            perturbance:0.03
        });
    ...
    //ScrollSpy
    $('a.elegant_scrollspy[href^="#"]:not([href="#"]').on('click', function(event){
